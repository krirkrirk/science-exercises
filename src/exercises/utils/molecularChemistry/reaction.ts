  ...
  static randomReaction(): Reaction {
    ...
    return new Reaction(
      selectedReaction.map((el) => {
        return { coefficient: el.coefficient, species: molecules.find((m) => m.formula === el.species) };
      }),
    );
  }
}
...
export class Reaction {
  ...
  getReactionWithoutCoef(): string {
    ...
    const reactantsString = reactants.map((r) => `? ${r.species?.formula}`).join(' + ');
    const productsString = products.map((r) => `? ${r.species?.formula}`).join(' + ');
  ...
  private getRandomCoefficient(maxRange: number = 6): number[] {
    ...
    while (verif) {
      ...
      for (let i = 0; i < myReactionCoef.length - 1; i++)
        if (myReactionCoef[i] / randomCoefs[i] !== myReactionCoef[i + 1] / randomCoefs[i + 1]) verif = false;
    }
  ...
  getReactionWithQuestionMark(specieIndexWithQuestionMark: number): string {
    ...
    if (specieIndexWithQuestionMark >= reactants.length) {
      ...
      reactantsString = reactants
        .map((r, index) => {
          const coefficient = Math.abs(r.coefficient) === 1 ? '' : Math.abs(r.coefficient);
          return index === specieIndexWithQuestionMark ? '\\ ?' : `${coefficient} ${r.species?.formula}`;
        })
        .join(' + ');
